...
import { BadRequestError, IAuthDocument, IEmailMessageDetails, lowerCase } from '@hoyci/jobber-shared';
...
export async function resendEmail(req: Request, res: Response): Promise<void> {
  ...
  const checkIfUserExists: IAuthDocument = await getAuthUserByEmail(lowerCase(email));
